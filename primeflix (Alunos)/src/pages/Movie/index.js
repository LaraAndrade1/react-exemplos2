...
function Movie() {
  ...
  return (
    <div className={styles.container}> {/* Contêiner principal */}
      <div className={styles.title} style={{ backgroundImage: `linear-gradient(to right, rgba(var(--primeflixDark), 0.9) 100%, rgba(var(--primeflixDark), 0) 100%), url('https://image.tmdb.org/t/p/original/${movie.backdrop_path}')` }}> {/* Título com imagem de fundo */}
        <div className={styles.grid_title}> {/* Grid para alinhar elementos */}
          <div className={styles.grid_title_img}> {/* Imagem do filme */}
            {
              movie.poster_path !== null ? ( // Verifica se há imagem do pôster
                <LazyLoadImage
          ...
          <div className={styles.area_buttons}> {/* Área de botões */}
